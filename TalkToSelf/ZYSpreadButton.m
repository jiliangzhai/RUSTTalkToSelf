...
- (instancetype)initWithBackgroundImage:(UIImage *)backgroundImage highlightImage:(UIImage *)highlightImage position:(CGPoint)position {
    ...
    self = [super init];
    if (self) {
        self.mainFrame = CGRectMake(position.x, position.y, 40, 40);
        self.superViewRelativePosition = position;
        
        //main Button
        self.powerButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 40, 40)];
            ...
            switch (_positionMode) {
                ...
                case SpreadPositionModeTouchBorder:
                {
                    ...
                    if (location.y < 90) {//上面区域
                        destinationLocation = CGPointMake(location.x, self.bounds.size.width/2 + _touchBorderMargin+20);
                    } else if (location.y > [UIScreen mainScreen].bounds.size.height - 90) {//下面
                        destinationLocation = CGPointMake(location.x, [UIScreen mainScreen].bounds.size.height - self.bounds.size.height/2 - _touchBorderMargin-40);
...
- (void)closeButton:(ZYSpreadSubButton *)exclusiveBtn {
    ...
    [UIView animateWithDuration:_animationDuring animations:^{
        ...
        for (ZYSpreadSubButton *button in self.subButtons) {
            button.center = self.powerButton.center;
        }
    }];
...
- (void)closeSubButton:(ZYSpreadSubButton *)exclusiveBtn {
    ...
    for (ZYSpreadSubButton *btn in _subButtons) {
        ...
        [CATransaction begin];
        [CATransaction setValue:(id)kCFBooleanTrue forKey:kCATransactionDisableActions];
        //btn.frame = CGRectMake(2.5,2.5, btn.bounds.size.width, btn.bounds.size.height);
        btn.layer.position = self.powerButton.layer.position;
        [CATransaction commit];
    }
    ...
    if (exclusiveBtn != nil) {
        ...
        exclusiveBtn.transform = CGAffineTransformMakeScale(1.0f, 1.0f);
        [CATransaction commit];*/
        
        [UIView animateWithDuration:_animationDuring delay:0.0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
            exclusiveBtn.alpha = 0.0;
            exclusiveBtn.transform = CGAffineTransformMakeScale(1.8, 1.8);
        } completion:^(BOOL finished) {
            exclusiveBtn.alpha = 0.0;
            exclusiveBtn.transform = CGAffineTransformMakeScale(1.0, 1.0);
        }];
    }
}
...
- (UIBezierPath *)movingPathWithStartPoint:(CGPoint)startPoint keyPoints:(NSValue *)keyPoints, ... {
    ...
    CGPoint point1 = [keyPoints CGPointValue];
    if (keyPoints) {
        NSValue *point;
        while ((point = va_arg(varList, NSValue *))) {
            [argArray addObject:point];
        }
    }
    va_end(varList);
